...
{$Region ' 文件比较 ' }
  ...
  TScanResultRemoveFolderInfo = class( TScanResultInfo )
    ...
    // 修改文件
  TScanResultModifyFileInfo = class( TScanResultInfo )
  end;
...
{$Region ' 文件传输 ' }
  ...
  protected    // 专为 Ios 文件监听设置
    function ReadCreateFilePath : string;virtual;
    procedure ReceiveFileAfter;virtual;
  end;
...
procedure TNetworkReceiveFileHandle.ReceiveFileAfter;
begin
...
begin
  try       // 创建写入流
    ...
    begin
      ForceDirectories( ExtractFileDir( ReadCreateFilePath ) );
      WriteStream := TFileStream.Create( ReadCreateFilePath, fmCreate or fmShareDenyNone );
    end;
    Result := True;
  except
    WriteStream := nil;
...
function TNetworkReceiveFileHandle.ReadCreateFilePath: string;
begin
