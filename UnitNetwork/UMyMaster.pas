...
{$Region ' 信息 数据结构 ' }
  ...
  published
    ...
    property ServerName : string Read iServerName Write iServerName;
    property EditionNumber : string Read iEditionNumber Write iEditionNumber;
  ...
  private
    iServerPcID, iServerPcName : string;
    iServerLanIp, iServerLanPort : string;
    iServerInternetIp, iServerInternetPort : string;
  public
    procedure SetPcInfo( _ServerPcID, _ServerPcName : string );
  ...
  published
    ...
    property ServerPcName : string Read iServerPcName Write iServerPcName;
...
begin
  ...
  if BroadcastType = BroadcastType_MacSearch then
  begin
    NetworkPcApi.SetSocketInfo( PcID, LanIp, LanPort, True );
    SendMobileReturn;
  end;
...
begin
  ...
  ServerInfoMsg.SetPcInfo( MyClient.ServerPcID, MasterName );
  ServerInfoMsg.SetLanInfo( MyClient.ServerLanIp, MyClient.ServerLanPort );
  ServerInfoMsg.SetInternetInfo( MyClient.ServerInternetIp, MyClient.ServerInternetPort );
  MsgStr := ServerInfoMsg.getMsg;
  ServerInfoMsg.Free;
...
function TMobileBroadcastReturnMsg.getMsgType: string;
begin
  Result := 'MobileBroadcastReturnMsg';
end;

procedure TMobileBroadcastReturnMsg.SetEditionNumber(_EditionNumber: string);
begin
  EditionNumber := _EditionNumber;
end;

procedure TMobileBroadcastReturnMsg.SetInternetInfo(_InternetIp,
  _InternetPort: string);
begin
  InternetIp := _InternetIp;
  InternetPort := _InternetPort;
...
procedure TMobileBroadcastReturnMsg.SetPcID(_PcID: string);
begin
  PcID := _PcID;
end;

procedure TMobileBroadcastReturnMsg.SetServerName(_ServerName: string);
begin
...
begin
  Result := TServerInfoMsg.ClassName;
end;
...
procedure TServerInfoMsg.SetPcInfo(_ServerPcID, _ServerPcName: string);
begin
  ServerPcID := _ServerPcID;
  ServerPcName := _ServerPcName;
end;

{ MobileMsgUtil }

class function MobileMsgUtil.getMsg: string;
var
...
begin
  ...
  MobileBroadcastReturnMsg.SetServerName( PcInfo.PcName );
  MobileBroadcastReturnMsg.SetEditionNumber( IntToStr( ConnEdition_Now ) );
  MobileBroadcastReturnMsg.SetInternetInfo( PcInfo.InternetIp, PcInfo.InternetPort );
  MobileBroadcastReturnMsg.SetPcID( PcInfo.PcID );
  Result := MobileBroadcastReturnMsg.getMsg;
