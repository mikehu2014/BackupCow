...
{$Region ' 搜索 Master 线程 ' }
  ...
  private
    ...
    procedure SendLanBeServerMobile;
    procedure SendGroupBeServerMobile;
    procedure SendGroupMobile( Ip, Port, MsgStr : string );
  end;
...
const
  ...
  HttpReq_EditionNumber = 'EditionNumber';
  HttpReq_DeviceType = 'DeviceType';
  ...
  UdpPort_Broadcast : Integer = 6542;
var
...
begin
  ...
  IsExistPc := ActivatePcList.Count > 1;  // 除本机外，是否还存在其他Pc
  ActivatePcList.Free;

     // 只有本机 且 没有本地备份， 则显示没有网络 Pc
  if not IsExistPc and not DesItemInfoReadUtil.ReadIsExistLocalBackup then
    NetworkErrorStatusApi.ShowNoPc;


  // 通知移动
  if SearchServerRun is TLanSearchServer then  // 局域网的情况
    SendLanBeServerMobile
  else
  if SearchServerRun is TGroupSearchServer then  // group 的情况
    SendGroupBeServerMobile;
end;
...
procedure TMasterThread.SendGroupBeServerMobile;
var
  MsgStr : string;
  GroupSearchServer : TGroupSearchServer;
  StandardPcHash : TStandardPcHash;
  p : TStandardPcPair;
begin
    // 成为服务器
  GroupSearchServer := SearchServerRun as TGroupSearchServer;
  GroupSearchServer.BeServer;

//    // 获取移动广播
//  MsgStr := MobileMsgUtil.getMsg;
//
//    // 发送
//  GroupSearchServer := SearchServerRun as TGroupSearchServer;
//  StandardPcHash := GroupSearchServer.StandardPcHash;
//  for p in StandardPcHash do
//    if p.Value.DeviceType = DeviceType_IOS then
//      SendGroupMobile( p.Value.LanIp, p.Value.LanPort, MsgStr );
end;

procedure TMasterThread.SendGroupMobile(Ip, Port, MsgStr: string);
var
  TcpSocket : TCustomIpClient;
begin
  // 连接并发送
...
begin
  ...
  if TcpSocket.Connect then
    TcpSocket.Sendln( MsgStr );
  TcpSocket.Free;
end).Start;
end;

procedure TMasterThread.SendLanBeServerMobile;
var
  MsgStr : string;
begin
  // 延迟发送
...
begin
  Sleep( 500 );
      ...
      // 发送移动广播
  MsgStr := MobileMsgUtil.getMsg;
  SendBroadcastUtil.SendMsg( MsgStr );
end).Start;
end;
...
procedure TGroupSearchServer.BeServer;
var
...
begin
  ...
  FindStandardNetworkHttp.SetCmd( Cmd_BeServer );
...
begin
  ...
  begin
    ...
    begin
      ...
      DeviceType := PcProStrList[ PcPro_DeviceType ];
      ...
      StandardPcInfo.SetDeviceType( DeviceType );
...
begin
  ...
  for p in StandardPcHash do
  begin
    if p.Value.DeviceType = DeviceType_IOS then  // 不连接移动设备
      Continue;
...
var
  ServerInfoStr, MsgStr : string;
  ServerInfoMsg : UMyMaster.TServerInfoMsg;
begin
  ServerInfoStr := MySocketUtil.RevData( TcpSocket );
  MsgStr := MsgUtil.getMsgStr( ServerInfoStr );

  ServerInfoMsg := UMyMaster.TServerInfoMsg.Create;
  ServerInfoMsg.SetMsg( MsgStr );
  ServerPcID := ServerInfoMsg.ServerPcID;
  ServerPcName := ServerInfoMsg.ServerPcName;
...
procedure TStandardPcInfo.SetDeviceType(_DeviceType: string);
begin
...
var
  ...
  LanIp, LanPort : string;
  InternetIp, InternetPort : string;
  CloudIDNumber, EditionNumber : string;
  params : TStringlist;
  idhttp : TIdHTTP;
  UrlStr : string;
begin
  ...
  EditionNumber := IntToStr( ConnEdition_Now );
  ...
  params.Add( HttpReq_EditionNumber + '=' + EditionNumber );
  params.Add( HttpReq_DeviceType + '=' + DeviceType_Pc );

  idhttp := TIdHTTP.Create(nil);
  idhttp.ReadTimeout := 30000;
  idhttp.ConnectTimeout := 30000;
  try
    UrlStr := MyUrl.getGroupPcList + '?cmd=' + Cmd;
    Result := idhttp.Post( UrlStr, params );
    DebugLog( 'UrlStr: ' + UrlStr );
    DebugLog('Params:' +  params.Text );
  except
