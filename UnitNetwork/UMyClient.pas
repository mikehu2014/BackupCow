...
uses Classes, Sockets, UChangeInfo, SyncObjs, UMyUtil, SysUtils,UMyNetPcInfo, DateUtils, UModelUtil,
     uDebug, UFileBaseInfo, uDebugLock, Forms;
...
{$Region ' Client 状态信息 ' }
  ...
  published
    ...
    property DeviceType : string Read iDeviceType Write iDeviceType;
  public
  ...
  private
    ...
    procedure SendMobileBoxFileList;
  end;
...
{$Region ' Client 网络备份信息 ' }
  ...
  TFileChangeNotifyMsg = class( TPcMsgBase )
  private
    iFileChangeStr : string;
  published
    property FileChangeStr : string Read iFileChangeStr Write iFileChangeStr;
  public
    procedure SetFileChangeStr( _FileChangeStr : string );
    function getMsgType : string;override;
  end;

  TFileChangeRenameMsg = class( TPcMsgBase )
  private
    iCloudPath : string;
    iOldName, iNewName : string;
  published
    property CloudPath : string Read iCloudPath Write iCloudPath;
    property OldName : string Read iOldName Write iOldName;
    property NewName : string Read iNewName Write iNewName;
  public
    procedure SetCloudPath( _CloudPath : string );
    procedure SetChangeName( _OldName, _NewName : string );
    procedure Update;override;
  public
    function getMsgType : string;override;
  end;
...
{$EndRegion}
  ...
  public
    procedure FileChangeHandle( s : string );  // 文件变化
  end;
...
const
  ...
  MsgType_NetworkBackup_FileChangeNotify = 'nb_fcn';
  MsgType_NetworkBackup_FileChangeRename = 'nb_fcr';
...
uses UMyServer,  UNetworkFace, UMyMaster, USearchServer, UMyTcp, UMyBackupApiInfo,
     ...
     UMobileBoxInfo, UMainApi,
     UMyCloudDataInfo, UMyCloudApiInfo, UBackupThread, UMyTimerThread, UFileNotifyForm;
...
procedure TMyClient.FileChangeHandle(s: string);
var
  IosList : TStringList;
  FileChangeNotifyMsg : TFileChangeNotifyMsg;
  i: Integer;
begin
  if not IsConnServer then
    Exit;

  IosList := MyNetPcInfoReadUtil.ReadIosList;
  for i := 0 to IosList.Count - 1 do
  begin
    FileChangeNotifyMsg := TFileChangeNotifyMsg.Create;
    FileChangeNotifyMsg.SetPcID( PcInfo.PcID );
    FileChangeNotifyMsg.SetFileChangeStr( s );
    MyClient.SendMsgToPc( IosList[i], FileChangeNotifyMsg );
  end;
  IosList.Free;
end;
...
begin
  ...
  ClientRevMsgThread.Resume;
    ...
    // 发送心跳 线程
  MyTimerHandler.AddTimer( HandleType_ClientHeartBeat, 20 );
  ...
  PcOnlineMsg.SetDeviceType( DeviceType_Pc );
...
begin
  ...
  PcBackOnlineMsg.SetDeviceType( DeviceType_Pc );
...
procedure TPcOnlineMsgBase.SendMobileBoxFileList;
var
  i : Integer;
  ...
  NotifyStr : string;
begin
  FileList := MobileBoxReadUtil.ReadPathList;
  for i := 0 to FileList.Count - 1 do
  begin
    FilePath := FileList[i];
    if FileExists( FilePath ) then
      NotifyStr := 'CREATE: '+ FilePath + '|' + IntToStr( MyFileInfo.getFileSize( FilePath ) ) +
                   '|' + DateTimeToStr( MyFileInfo.getFileLastWriteTime( FilePath ) )
    else
      NotifyStr := 'MKDIR: ' + FilePath;
    MyClient.FileChangeHandle( NotifyStr );
  end;
  FileList.Free;
end;

procedure TPcOnlineMsgBase.SetDeviceType(_DeviceType: string);
begin
...
begin
  ...
  NetworkPcApi.AddItem( PcID, PcName );
    ...
    // 设置 Pc 的设备类型
  NetworkPcApi.SetDeviceType( PcID, DeviceType );

    // 添加 移动盒子
  if DeviceType = DeviceType_IOS then
  begin
    SendMobileBoxFileList;
    MyCloudPathUserApi.AddMoblieBox;
    if not Assigned( frmFileNotify ) then
    begin
      Notify_Path := MyCloudInfoReadUtil.ReadMobileBoxPath;
      t := TThread.CurrentThread;
      t.Synchronize( t,
      procedure
      begin
        Application.CreateForm(TfrmFileNotify, frmFileNotify);
        frmFileNotify.OnFileChange := MyClient.FileChangeHandle;
      end);
    end;
    MyHintAppApi.ShowMoblieBoxBtn;
  end;
...
begin
  ...
  if MsgType = MsgType_NetworkBackup_FileChangeNotify then
    Result := TFileChangeNotifyMsg.Create
  else
  if MsgType = MsgType_NetworkBackup_FileChangeRename then
    Result := TFileChangeRenameMsg.Create
  else
    Result := nil;
end;
...
{ TFileChangeNotifyMsg }

function TFileChangeNotifyMsg.getMsgType: string;
begin
  Result := MsgType_NetworkBackup_FileChangeNotify;
end;

procedure TFileChangeNotifyMsg.SetFileChangeStr(_FileChangeStr: string);
begin
  FileChangeStr := _FileChangeStr;
end;

{ TFileChangeRenameMsg }

function TFileChangeRenameMsg.getMsgType: string;
begin
  Result := MsgType_NetworkBackup_FileChangeRename;
end;

procedure TFileChangeRenameMsg.SetChangeName(_OldName, _NewName: string);
begin
  OldName := _OldName;
  NewName := _NewName;
end;

procedure TFileChangeRenameMsg.SetCloudPath(_CloudPath: string);
begin
  CloudPath := _CloudPath;
end;

procedure TFileChangeRenameMsg.Update;
var
  NewFilePath, OldFilePath : string;
begin
  NewFilePath := MyCloudInfoReadUtil.ReadCloudFilePath( CloudPath, PcID, NewName );
  OldFilePath := MyCloudInfoReadUtil.ReadCloudFilePath( CloudPath, PcID, OldName );
