...
{$Region ' 写 端口映射 ' }
  ...
  TNetPortMapWriteXml = class( TNetPcChangeXml )
  private
    IsPortMap : Boolean;
  public
    procedure SetIsPortMap( _IsPortMap : Boolean );
  protected
    procedure Update;override;
  end;
...
begin
  ...
  begin
    ...
    // 读取上一次运行是否端口映射成功
  IsPortMapStr := MyXmlUtil.GetChildValue( MyPcNode, Xml_IsPortMap );
  if IsPortMapStr <> '' then
    Read_IsPortMap := StrToBoolDef( IsPortMapStr, True );
end;

{ TNetPortMapWriteXml }

procedure TNetPortMapWriteXml.SetIsPortMap(_IsPortMap: Boolean);
begin
  IsPortMap := _IsPortMap;
end;

procedure TNetPortMapWriteXml.Update;
begin
  inherited;

  Read_IsPortMap := IsPortMap;
  MyXmlUtil.AddChild( MyNetPcInfoXml, Xml_IsPortMap, IsPortMap );
end;
