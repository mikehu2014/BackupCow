...
begin
  ...
  begin
    ...
    PmNetwork.Items[i].Default := False;
  end;

  NetworkModeShowStr := 'Network Mode: ';
  if SelectType = SelectConnType_Local then
    NetworkModeShowStr := NetworkModeShowStr + 'Local Network'
  else
  if SelectType = SelectConnType_Group then
    NetworkModeShowStr := NetworkModeShowStr + 'Group ( ' + SelectValue1 + ' )'
  else
  if SelectType = SelectConnType_ConnPC then
    NetworkModeShowStr := NetworkModeShowStr + 'Computer ( ' + SelectValue1 + ' )';
  frmMainForm.tbtnBackupNetwork.Caption := NetworkModeShowStr;
end;
