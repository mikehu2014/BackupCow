...
{$EndRegion}
  ...
  public
    class function RevJsonStr( TcpSocket : TCustomIpClient ) : string;overload;
    class function RevJsonStr( TcpSocket : TCustomIpClient; WaitTime : Integer ) : string;overload;
...
begin
  ...
  begin
    ...
    // 连上了 但出现 连接错误
  RevStr := MySocketUtil.RevJsonStr( TcpSocket, 10 );
  if RevStr <> ConnResult_OK then
  begin
...
begin
    // 发送大字符串需要多次发送
//  if ( Length( AnsiString( MsgStr ) ) > 500 ) and ( TcpSocket.Tag <> 5 ) then
...
var
  FormLogAddFace : TFormLogAddSendFace;
begin
  try
    ...
    // 是否记录接收信息
  if not IsMark_Send then
    Exit;

    // 写 log
  FormLogAddFace := TFormLogAddSendFace.Create( MsgStr );
  FormLogAddFace.AddChange;
end;
...
class function MySocketUtil.RevJsonStr(TcpSocket: TCustomIpClient;
  WaitTime: Integer): string;
var
  MsgInfo : TMsgInfo;
begin
  try
    MsgInfo := TMsgInfo.Create;
    Result := RevData( TcpSocket, WaitTime );
