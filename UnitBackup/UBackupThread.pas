...
{$Region ' 数据结构 ' }
    ...
    // 备份路径信息
  TBackupJobInfo = class
  public
    DesItemID : string;  // 目标信息
    BackupPath : string; // 源路径
  public
    procedure SetItemInfo( _DesItemID, _BackupPath : string );
  end;
  TLocalBackupPathInfo = class( TBackupJobInfo )end;
  TNetworkBackupPathInfo = class( TBackupJobInfo )end;
  TBackupJobList = class( TObjectList<TBackupJobInfo> )end;

    // 日志信息
  TLogPathInfo = class( TBackupJobInfo )
  public
    FilePath : string;
    FileTime : TDateTime;
  public
...
{$Region ' 文件备份 ' }
  ...
  public
    constructor Create( _BackupPathInfo : TBackupJobInfo );
    procedure Update;
  private
  ...
  protected
    procedure Execute; override;
  public          // 扫描
    procedure StartBackup( BackupPathInfo : TBackupJobInfo );
    procedure StopBackup( BackupPathInfo : TBackupJobInfo );
  private        // 免费版试用限制
  ...
  private
    ...
    BackupJobList : TBackupJobList;
    IsCreateThread : Boolean;
    BackupHandleThread : TBackupHandleThread;
  private
    BackupTimerList : TBackupTimerList;
  public
    constructor Create;
    procedure StopScan;
    destructor Destroy; override;
  public
    ...
    procedure ShowFreeLimit;
    procedure AddBackupJob( BackupJobInfo : TBackupJobInfo );
    function getBackupJob : TBackupJobInfo;
  end;
...
{$Region ' Mac处理 ' }
  ...
  TBackupTimerInfo = class
  public
    BackupPath : string;
    BackupSize : Int64;
  public
    constructor Create( _BackupPath : string; _BackupSize : Int64 );
    function ReadIsBackup( NewSize : Int64 ): Boolean;
  end;
  TBackupTimerList = class( TObjectList<TBackupTimerInfo> )
  public
    function ReadIsBackup( FilePath : string; NewSize : Int64 ): Boolean;
  end;
...
begin
  ...
  begin
    ScanPathInfo := MyBackupHandler.getBackupJob;
...
procedure TBackupHandleThread.StartBackup(BackupPathInfo: TBackupJobInfo);
var
...
procedure TBackupHandleThread.StopBackup(BackupPathInfo: TBackupJobInfo);
begin
  BackupItemAppApi.SetStopBackup( BackupPathInfo.DesItemID, BackupPathInfo.BackupPath );
  BackupPathInfo.Free;
end;
...
procedure TMyBackupHandler.AddBackupJob(
...
begin
  ...
  ThreadLock.Enter;
    ...
    // 添加到扫描列表中
  BackupJobList.Add( BackupJobInfo );
...
begin
  ...
  BackupJobList := TBackupJobList.Create;
  BackupJobList.OwnsObjects := False;
  IsCreateThread := False;

  IsBackupRun := True;
  IsRun := True;
  BackupTimerList := TBackupTimerList.Create;
end;

destructor TMyBackupHandler.Destroy;
begin
  BackupTimerList.Free;
  BackupJobList.OwnsObjects := True;
  BackupJobList.Free;
  ThreadLock.Free;
  inherited;
end;
...
function TMyBackupHandler.getBackupJob: TBackupJobInfo;
begin
  ThreadLock.Enter;
  if BackupJobList.Count > 0 then
  begin
    Result := BackupJobList[0];
    BackupJobList.Delete(0);
  end
  else
  begin
    Result := nil;
...
procedure TBackupJobInfo.SetItemInfo(_DesItemID, _BackupPath: string);
begin
...
begin
  ...
  if MyNetPcInfoReadUtil.ReadIsMac( DesPcID ) and
     not MyBackupHandler.BackupTimerList.ReadIsBackup( SourcePath, TotalSize )
  then
  begin
...
begin
  ...
  AnalyzeResetHandle.Free;
    ...
    // 设置已完成备份
  BackupHintAppApi.SetBackupReal( True );
end;
...
begin
  try
    ...
    if ScanResultInfo is TScanResultAddFileInfo then  // 添加文件
        SourceFileAdd
    else
    if ScanResultInfo is TScanResultAddFolderInfo then  // 添加目录
      SourceFolderAdd
    else
    if ScanResultInfo is TScanResultAddZipInfo then  // 添加压缩文件
      SourceFileAddZip
    else
    if ScanResultInfo is TScanResultRemoveFileInfo then  // 删除文件
    begin
    ...
    if ScanResultInfo is TScanResultRemoveFolderInfo then  // 删除目录
    begin
      if IsSaveDeleted then
        DesFolderRecycle
      else
...
begin
  ...
  CheckHeartBeat;
    ...
    // Mac 不压缩
  if MyNetPcInfoReadUtil.ReadIsMac( DesPcID ) then
  begin
...
constructor TBackupStartHandle.Create(_BackupPathInfo: TBackupJobInfo);
begin
  BackupPathInfo := _BackupPathInfo;
  DesItemID := BackupPathInfo.DesItemID;
  SourcePath := BackupPathInfo.BackupPath;
...
begin
  ...
  SourcePath := LogPathInfo.BackupPath;
...
constructor TBackupTimerInfo.Create(_BackupPath: string;
  _BackupSize : Int64);
begin
  BackupPath := _BackupPath;
...
function TBackupTimerInfo.ReadIsBackup(NewSize: Int64): Boolean;
begin
  Result := BackupSize <> NewSize;
  if Result then
    BackupSize := NewSize;
end;
...
function TBackupTimerList.ReadIsBackup(FilePath: string;
  NewSize: Int64): Boolean;
var
  i: Integer;
  IsFind : Boolean;
  BackupTimerInfo : TBackupTimerInfo;
begin
  IsFind := False;
  Result := True;
  for i := 0 to Self.Count - 1 do
    if Self[i].BackupPath = FilePath then
    begin
      IsFind := True;
      Result := Self[i].ReadIsBackup( NewSize );
      Break;
    end;
  if not IsFind then
  begin
    BackupTimerInfo := TBackupTimerInfo.Create( FilePath, NewSize );
    Self.Add( BackupTimerInfo );
  end;
end;
