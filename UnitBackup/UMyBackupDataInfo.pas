...
{$Region ' 备份信息 数据修改 ' }
  ...
  public
    ...
    // 设置 同步周期
  TBackupItemSetScheduleInfo = class( TBackupItemWriteInfo )
  private
    SchduleType : Integer;
...
{$Region ' 目标信息 数据读取 ' }
  ...
  public
    ...
    // 读取 备份总文件数
  TDesItemListReadTotalCount = class( TDesItemListAccessInfo )
  public
  ...
  public
    class function ReadTotalCount : Integer;
...
procedure TBackupItemInfo.SetScheduleInfo(_ScheduleType, _ScheduleValue1,
...
procedure TBackupItemAddInfo.SetScheduleInfo(_ScheduleType, _ScheduleValue1,
  _ScheduleValue2: Integer);
begin
  ScheduleType := _ScheduleType;
  ScheduleValue1 := _ScheduleValue1;
...
class function DesItemInfoReadUtil.ReadTotalCount: Integer;
var
  DesItemListReadTotalCount : TDesItemListReadTotalCount;
begin
  DesItemListReadTotalCount := TDesItemListReadTotalCount.Create;
  Result := DesItemListReadTotalCount.get;
  DesItemListReadTotalCount.Free;
end;
...
var
  SyncMins : Integer;
begin
  BackupItemInfo.NextSyncTime := ScheduleUtil.getNextBackupTime( BackupItemInfo.ScheduleType, BackupItemInfo.ScheduleValue1, BackupItemInfo.ScheduleValue2, BackupItemInfo.LasSyncTime );
...
begin
  ...
  Result.SetScheduleInfo( BackupItemInfo.ScheduleType, BackupItemInfo.ScheduleValue1, BackupItemInfo.ScheduleValue2 );
...
begin
  ...
  begin
    ...
    if not BackupItemInfo.IsAutoSync then // 非自动同步
      Continue;
    if BackupItemInfo.ScheduleType = ScheduleType_Manual then  // 手动同步
      Exit;
...
{ TDesItemListReadTotalCount }

function TDesItemListReadTotalCount.get: Integer;
var
  i, j : Integer;
begin
  Result := 0;
  for i := 0 to DesItemList.Count - 1 do
    for j := 0 to DesItemList[i].BackupItemList.Count - 1 do
      Result := Result + DesItemList[i].BackupItemList[j].FileCount;
end;

{ TBackupItemSetScheduleInfo }

procedure TBackupItemSetScheduleInfo.SetSchduleType(_SchduleType: Integer);
begin
  SchduleType := _SchduleType;
end;

procedure TBackupItemSetScheduleInfo.SetSchduleValue(_SchduleValue1,
...
procedure TBackupItemSetScheduleInfo.Update;
begin
  if not FindBackupItemInfo then
    Exit;
