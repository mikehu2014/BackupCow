...
{$Region ' 数据修改 ' }
  ...
  TBackupSetReallyXml = class( TXmlChangeInfo )
  private
    IsRealBackup : Boolean;
  public
    procedure SetIsRealBackup( _IsRealBackup : Boolean );
  protected
  ...
  protected
    procedure Update;override;
      ...
      // 设置 同步周期
  TBackupItemSetScheduleXml = class( TBackupItemWriteXml )
  private
    SchduleType : Integer;
...
{$Region ' 数据读取 ' }
  ...
  private
    procedure ReadScheduleInfo( var ScheduleType, ScheduleValue1, ScheduleValue2 : Integer );
  end;
...
procedure TBackupItemAddXml.SetScheduleInfo(_ScheduleType, _ScheduleValue1,
...
begin
  ...
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleType, ScheduleType );
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleValue1, ScheduleValue1 );
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleValue2, ScheduleValue2 );
...
begin
  ...
  Backup_Really := MyXmlUtil.GetChildBoolValue( MyBackupNode, Xml_RealBackup );
end;
...
procedure TBackupItemReadXml.ReadScheduleInfo( var ScheduleType,
  ScheduleValue1, ScheduleValue2 : Integer );
var
...
begin
  ...
  ScheduleValue2 := 0;
    ...
    // 不需要自动同步
  if not IsAutoSync then
    Exit;

    // 版本兼容
  if SyncTimeType = TimeType_Minutes then
  begin
    ScheduleType := ScheduleType_Min;
    if SyncTimeValue <= 1 then
      ScheduleValue1 := 0
    else
    if SyncTimeValue < 10 then
      ScheduleValue1 := 1
    else
    if SyncTimeValue < 30 then
      ScheduleValue1 := 2
    else
      ScheduleValue1 := 5;
  end
  else
  if SyncTimeType = TimeType_Hourse then
  begin
    ScheduleType := ScheduleType_Hour;
    if SyncTimeValue < 7 then
      ScheduleValue1 := SyncTimeValue - 1
    else
      ScheduleValue1 := 6;
  end
  else
  if SyncTimeType = TimeType_Day then
  begin
    ScheduleType := ScheduleType_Day;
    ScheduleValue1 := 10;
  end
  else
  if SyncTimeType = TimeType_Week then
  begin
    ScheduleType := ScheduleType_Week;
    ScheduleValue1 := 0;
    ScheduleValue2 := 10;
  end
  else
  if SyncTimeType = TimeType_Month then
  begin
    ScheduleType := ScheduleType_Month;
    ScheduleValue1 := 0;
    ScheduleValue2 := 10;
  end;
end;
...
begin
  ...
  ScheduleTypeStr := MyXmlUtil.GetChildValue( BackupItemNode, Xml_ScheduleType );
  if ScheduleTypeStr <> '' then
  begin
    ScheduleType := MyXmlUtil.GetChildIntValue( BackupItemNode, Xml_ScheduleType );
    ScheduleValue1 := MyXmlUtil.GetChildIntValue( BackupItemNode, Xml_ScheduleValue1 );
    ScheduleValue2 := MyXmlUtil.GetChildIntValue( BackupItemNode, Xml_ScheduleValue2 );
  end
  else
    ReadScheduleInfo( ScheduleType, ScheduleValue1, ScheduleValue2 );
  ...
  BackupItemReadHandle.SetScheduleInfo( ScheduleType, ScheduleValue1, ScheduleValue2 );
...
{ TBackupItemSetScheduleXml }

procedure TBackupItemSetScheduleXml.SetSchduleType(_SchduleType: Integer);
begin
  SchduleType := _SchduleType;
end;

procedure TBackupItemSetScheduleXml.SetSchduleValue(_SchduleValue1,
...
procedure TBackupItemSetScheduleXml.Update;
begin
  inherited;
  if not FindBackupItemNode then
    Exit;
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleType, SchduleType );
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleValue1, SchduleValue1 );
  MyXmlUtil.AddChild( BackupItemNode, Xml_ScheduleValue2, SchduleValue2 );
end;

{ TBackupSetReallyXml }

procedure TBackupSetReallyXml.SetIsRealBackup(_IsRealBackup: Boolean);
begin
  IsRealBackup := _IsRealBackup;
end;

procedure TBackupSetReallyXml.Update;
var
  MyBackupNode : IXMLNode;
begin
  MyBackupNode := MyXmlUtil.AddChild( MyXmlDoc.DocumentElement, Xml_MyBackupInfo );
  MyXmlUtil.AddChild( MyBackupNode, Xml_RealBackup, IsRealBackup );
  Backup_Really := IsRealBackup;
end;
