...
type
  ...
  TfrmBackupHint = class(TForm)
    ...
    procedure lvFileBackupDblClick(Sender: TObject);
  private
    DesItemID, BackupPath : string;
  public
...
procedure TfrmBackupHint.lvFileBackupDblClick(Sender: TObject);
var
  ItemData : TLvHintData;
begin
  if not Assigned( lvFileBackup.Selected ) then
    Exit;
  ItemData := lvFileBackup.Selected.Data;
  if ItemData.FilePath = '' then
    btnShowLog.Click
  else
    MyExplore.OpenFolder( ItemData.FilePath );
end;
...
begin
  ...
  lbTitle.Caption := 'Backup Completed';
  ...
  lbBackupTo.Caption := Params.BackupTo;
    ...
    // 显示已备份文件
  lbBackupFiles.Caption := MyCount.getCountStr( Params.TotalBackup ) + ' files has been backed up';
  ...
  for i := 0 to BackupFileList.Count - 1 do
    ...
    begin
      ...
      SubItemImages[0] := MyShellTransActionIconUtil.getLoaded;
      Data := TLvHintData.Create( BackupFileList[i] );
    end;
  if BackupFileList.Count = 10 then
    with lvFileBackup.Items.Add do
    begin
      Caption := '...';
      SubItems.Add( '' );
