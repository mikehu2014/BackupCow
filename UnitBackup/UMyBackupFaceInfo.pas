...
{$Region ' 备份信息 数据修改 ' }
  ...
  private
    PcName, Path : string;
    IsOnline, IsLan : Boolean;
    DeviceType : string;
  public
    procedure SetDesInfo( _PcName, _Path : string );
  ...
  public
    ...
    procedure SetIsOnline( _IsOnline : Boolean );
    procedure SetIsLanPc( _IsLanPc : Boolean );
  protected
    procedure Update;override;
  private
  ...
  public
    class function ReadPcIcon( IsOnline, IsLanPc : Boolean; DesItemID, DeviceType : string ): Integer;
  end;
  ...
  protected
    ...
    // 设置 同步周期
  TBackupItemSetScheduleFace = class( TBackupItemWriteFace )
  private
    SchduleType : Integer;
...
{$Region ' 选择窗口 数据修改 ' }
  ...
  public
    DesItemID : WideString;
    IsLocal : Boolean;  // 本地/网络目标
    MainIcon : Integer;
    AvailaleSpace : Int64;
  public                // 网络节点属性
    IsOnline : Boolean;
  ...
  protected
    ...
    // 添加 父类
  TFrmNetworkDesAddBase = class( TFrmNetworkDesWrite )
  public
    AvailableSpace : Int64;
  public
    procedure SetAvailableSpace( _AvailableSpace : Int64 );
      ...
      // 添加 本地 Item
  TFrmLocalDesAdd = class( TFrmNetworkDesAddBase )
  public
    ShowName : string;
    IsSelect : Boolean;
  public
  ...
  private
    ...
    // 添加 网络 Item
  TFrmNetworkDesAdd = class( TFrmNetworkDesAddBase )
  public
    MainName, DesName : string;
    IsOnline : Boolean;
    IsLanPc : Boolean;
  public
    procedure SetShowInfo( _MainName, _DesName : string );
    procedure SetIsOnline( _IsOnline : Boolean );
    procedure SetIsLanPc( _IsLanPc : Boolean );
  protected
    procedure Update;override;
  ...
  public
    class function ReadIcon( IsOnline, IsLanPc : Boolean ): Integer;
    class function ReadFilterIcon( IsOnline, IsLanPc : Boolean ): Integer;
...
const
  ...
  BackupIcon_IphoneOnline = 7;
  BackupIcon_MacOnline = 8;
  BackupIcon_MyComputer = 9;
  BackupIcon_NetworkFolder = 10;
  BackupIcon_LanComputer = 11;
  BackupIcon_InternetComputer = 12;
  BackupIcon_GrayComputer = 13;
  ...
  BackupStatusShow_Completed = 'Backup Completed';
...
const
  FilterIcon_PcOffline = 0;
  FilterIcon_MyComputer = 1;
  FilterIcon_SharedFolder = 2;
  FilterIcon_LanComputer = 3;
  FilterIcon_InternetComputer = 4;

  FrmDesIcon_PcOffline = 0;
  FrmDesIcon_MyComputer = BackupIcon_MyComputer;
  FrmDesIcon_SharedFolder = BackupIcon_NetworkFolder;
  FrmDesIcon_LanComputer = BackupIcon_LanComputer;
  FrmDesIcon_InternetComputer = BackupIcon_InternetComputer;
...
procedure TBackupItemAddFace.SetScheduleInfo(_ScheduleType, _ScheduleValue1,
...
begin
  ...
  begin
    ...
    // 修改节点
  BackupItemData.MainName := BackupPath;
  ...
  BackupItemData.ScheduleType := ScheduleType;
  BackupItemData.ScheduleValue1 := ScheduleValue1;
  BackupItemData.ScheduleValue2 := ScheduleValue2;
...
procedure TFrmNetworkDesAdd.SetIsLanPc(_IsLanPc: Boolean);
begin
  IsLanPc := _IsLanPc;
end;
...
procedure TFrmNetworkDesAdd.SetShowInfo(_MainName, _DesName: string);
begin
  MainName := _MainName;
  DesName := _DesName;
end;
...
begin
  ...
  begin
    ...
    NetworkDesData.IsLocal := False;
  end;
    // 刷新信息
  NetworkDesData.IsOnline := IsOnline;
  NetworkDesData.MainIcon := FrmNetworkDesUtil.ReadIcon( IsOnline, IsLanPc );
...
begin
  ...
  if NodeData.AvailableSpace >= 0 then
    Result := MyShellTransActionIconUtil.getNetworkSpace;
end;
...
begin
  ...
  begin
    ShowTime := ScheduleUtil.getNextBackupTime( ScheduleType, ScheduleValue1, ScheduleValue2, NodeData.LastSyncTime );
    ShowStr := FormatDateTime( 'mm-dd  hh:nn', ShowTime );
  end;
  NodeData.NextSyncTimeShow := ShowStr;
end;
...
begin
  ...
  DesItemData.DesName := Format( DesName_Show, [DesItemID] );
  if MyNetworkFolderUtil.IsNetworkFolder( DesItemID ) then
  begin
    DesItemData.MainName := BackupShow_SharedFolder;
    DesItemData.MainIcon := BackupIcon_NetworkFolder;
  end
  else
  begin
    DesItemData.MainName := BackupShow_MyComputer;
    DesItemData.MainIcon := BackupIcon_MyComputer;
  end;
...
begin
  ...
  DesItemData.MainName := PcName;
  DesItemData.DesName := Format( DesName_Show, [Path] );
  DesItemData.DeviceType := DeviceType;
  DesItemData.MainIcon := DesItemFaceUtil.ReadPcIcon( IsOnline, IsLan, DesItemID, DeviceType );
...
procedure TDesItemAddNetworkFace.SetDeviceType(_DeviceType: string);
begin
  DeviceType := _DeviceType;
end;

procedure TDesItemAddNetworkFace.SetIsLan(_IsLan: Boolean);
begin
  IsLan := _IsLan;
end;
...
procedure TDesItemAddNetworkFace.SetDesInfo(_PcName, _Path: string);
begin
  PcName := _PcName;
  Path := _Path;
end;
...
begin
  ...
  Result := MyHtmlHintShowStr.getHintRowNext( 'Backup Schedule', ShowStr1 );
  if ShowType2 <> '' then
  ...
  if ScheduleType <> ScheduleType_Manual then
    Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Next Scheduled Backup', DateTimeToStr( ScheduleUtil.getNextBackupTime( ScheduleType, ScheduleValue1, ScheduleValue2, NodeData.LastSyncTime )));
  Result := Result + '<br />';
...
begin
  ...
  Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Backup To', DestinationStr );
  ...
  Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Total File Size', MySize.getFileSizeStr( NodeData.ItemSize ));
  if NodeData.LastSyncTime <> 0 then
    Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Last Backup', DateTimeToStr( NodeData.LastSyncTime ) );
  Result := Result + '<br />';
end;
...
begin
  Result := MyHtmlHintShowStr.getHintRowNext( 'Keep Deleted/Replaced Files', MyBoolean.getBooleanStr( NodeData.IsSaveDeleted ) );
  if NodeData.IsSaveDeleted then
    Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Keep Versions', IntToStr( NodeData.SaveDeletedCount ) );
  Result := Result + '<br />';
end;
...
procedure TDesItemSetPcIsOnlineFace.SetIsLanPc(_IsLanPc: Boolean);
begin
  IsLanPc := _IsLanPc;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        NodeData.MainIcon := DesItemFaceUtil.ReadPcIcon( IsOnline, IsLanPc, NodeData.ItemID, NodeData.DeviceType );
...
class function DesItemFaceUtil.ReadPcIcon(IsOnline, IsLanPc: Boolean;
  DesItemID, DeviceType : string): Integer;
begin
  if IsOnline then
  begin
    if DeviceType = DeviceType_IOS then
      Result := BackupIcon_IphoneOnline
    else
    if DeviceType = DeviceType_mac then
      Result := BackupIcon_MacOnline
    else
    if IsLanPc then
      Result := BackupIcon_LanComputer
    else
      Result := BackupIcon_InternetComputer;
  end
  else
...
procedure TFrmNetworkDesIsOnline.SetIsLanPc(_IsLanPc: Boolean);
begin
  IsLanPc := _IsLanPc;
end;
...
begin
  ...
  begin
    ...
    if not NodeData.IsLocal and ( SelectPcID = DesPcID ) and ( IsOnline <> NodeData.IsOnline ) then
    begin
        // 位置发生变化，需要变量转换
      ChangeNode := SelectNode;
      SelectNode := SelectNode.NextSibling;
      ...
      NodeData.IsOnline := IsOnline;
      NodeData.MainIcon := FrmNetworkDesUtil.ReadIcon( IsOnline, IsLanPc );
      ChangeNode.NodeHeight := FrmNetworkDesUtil.ReadHeight( IsOnline );
...
class function FrmNetworkDesUtil.ReadFilterIcon(IsOnline,
  IsLanPc: Boolean): Integer;
begin
  if IsOnline then
  begin
    if IsLanPc then
      Result := FilterIcon_LanComputer
    else
      Result := FilterIcon_InternetComputer;
  end
  else
    Result := FilterIcon_PcOffline;
end;
...
class function FrmNetworkDesUtil.ReadIcon(IsOnline, IsLanPc: Boolean): Integer;
begin
  if IsOnline then
  begin
    if IsLanPc then
      Result := FrmDesIcon_LanComputer
    else
      Result := FrmDesIcon_InternetComputer;
  end
  else
...
begin
  ...
  begin
    ...
    DesItemData.DeviceType := DeviceType_Pc;
...
begin
  ...
  BackupPcFilterData.MainIcon := FrmNetworkDesUtil.ReadFilterIcon( IsOnline, True );
...
begin
  ...
  begin
    ...
    begin
      ...
      NodeData.IsOnline := IsOnline;
      NodeData.MainIcon := FrmNetworkDesUtil.ReadFilterIcon( IsOnline, True );
...
{ TFrmNetworkDesAddBase }

procedure TFrmNetworkDesAddBase.SetAvailableSpace(_AvailableSpace: Int64);
begin
  AvailableSpace := _AvailableSpace;
end;
...
procedure TFrmLocalDesAdd.CreateDesNode;
var
  LastLocalNode : PVirtualNode;
begin
  LastLocalNode := getLastLocalNode;
  if Assigned( LastLocalNode ) then  // 最后一个本地节点
    NetworkDesNode := vstNetworkDes.InsertNode( LastLocalNode, amInsertAfter )
  else  // 第一个节点
    NetworkDesNode := vstNetworkDes.InsertNode( vstNetworkDes.RootNode, amAddChildFirst );
end;

function TFrmLocalDesAdd.getLastLocalNode: PVirtualNode;
var
  SelectNode : PVirtualNode;
  NodeData : PNetworkDesData;
begin
...
procedure TFrmLocalDesAdd.SetShowName(_ShowName: string);
begin
  ShowName := _ShowName;
end;
...
begin
  ...
  begin
    ...
    NetworkDesData := vstNetworkDes.GetNodeData( NetworkDesNode );
    NetworkDesData.DesItemID := DesItemID;
    NetworkDesData.IsLocal := True;
  end;
  NetworkDesData.IsOnline := True;
  NetworkDesData.AvailaleSpace := AvailableSpace;

    // 设置目录图标
  if MyNetworkFolderUtil.IsNetworkFolder( DesItemID ) then
  begin
    NetworkDesData.MainName := BackupShow_SharedFolder;
    NetworkDesData.DesName := Format( DesName_Show, [ ShowName ] );
    NetworkDesData.MainIcon := FrmDesIcon_SharedFolder;
  end
  else
  begin
    NetworkDesData.MainName := BackupShow_MyComputer;
    NetworkDesData.DesName := Format( DesName_Show, [ ShowName ] );
    NetworkDesData.MainIcon := FrmDesIcon_MyComputer;
  end;

    // 选中目录
  if IsSelect then
    vstNetworkDes.CheckState[ NetworkDesNode ] := csCheckedNormal;

    // 刷新节点
  vstNetworkDes.RepaintNode( NetworkDesNode );
end;

{ TBackupItemSetScheduleInfo }

procedure TBackupItemSetScheduleFace.SetSchduleType(_SchduleType: Integer);
begin
  SchduleType := _SchduleType;
end;

procedure TBackupItemSetScheduleFace.SetSchduleValue(_SchduleValue1,
...
procedure TBackupItemSetScheduleFace.Update;
begin
  inherited;
  ...
  BackupItemData.ScheduleType := SchduleType;
  BackupItemData.ScheduleValue1 := SchduleValue1;
  BackupItemData.ScheduleValue2 := SchduleValue2;
