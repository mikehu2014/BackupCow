...
uses
  ...
  RzTabs, Vcl.ImgList, Vcl.ComCtrls, Vcl.ToolWin, UMainForm, IniFiles, UFormUtil;
...
begin
  ...
  vstBackupLog.PopupMenu := FormUtil.getPopMenu( tbCompleted );
...
begin
  ...
  spBtn1.Visible := tbtnPreview.Visible;
  tbtnRestore.Enabled := LogFileUtil.getIsRestore( Node );
  tbtnRestore.Visible := tbtnRestore.Enabled;
  tbtnExplorer.Enabled := LogFileUtil.getIsExplorer( Node );
  tbtnExplorer.Visible := tbtnExplorer.Enabled;
