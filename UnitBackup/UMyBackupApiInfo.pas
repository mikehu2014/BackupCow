...
{$Region ' 源路径 同步信息 ' }
  ...
  TBackupItemSetScheduleHandle = class( TBackupItemWriteHandle )
  public
    ScheduleType : Integer;
    ScheduleValue1, ScheduleValue2 : Integer;
  public
...
{$EndRegion}
  ...
  public             // 修改 配置
    ...
    class procedure SetScheduleInfo( Params : TBackupScheduleParams );
  ...
  public
    class procedure SetBackupReal( IsBackupReal : Boolean );
  end;
...
uses UMyBackupDataInfo, UMyBackupFaceInfo, UMyBackupXmlInfo, UMyNetPcInfo, UMyUtil, UBackupThread,
     ...
     UFormBackupLog, UXmlUtil, XMLIntf, UChangeInfo, UMainFormFace, UMyCloudApiInfo, UStorageApi;
...
begin
  ...
  BackupItemAddHandle.SetAutoSyncInfo( BackupConfigInfo.IsAuctoSync, 0 );
  BackupItemAddHandle.SetSyncTimeInfo( BackupConfigInfo.SyncTimeType, BackupConfigInfo.SyncTimeValue );
  BackupItemAddHandle.SetScheduleInfo( BackupConfigInfo.ScheduleType, BackupConfigInfo.ScheduleValue1, BackupConfigInfo.ScheduleValue2 );
...
class procedure BackupItemUserApi.SetScheduleInfo(
  Params: TBackupScheduleParams);
var
  BackupItemSetScheduleHandle : TBackupItemSetScheduleHandle;
begin
  BackupItemSetScheduleHandle := TBackupItemSetScheduleHandle.Create( Params.DesItemID );
  BackupItemSetScheduleHandle.SetBackupPath( Params.BackupPath );
  BackupItemSetScheduleHandle.SetScheduleType( Params.ScheduleType );
  BackupItemSetScheduleHandle.SetScheduleValue( Params.ScheduleValue1, Params.ScheduleValue2 );
  BackupItemSetScheduleHandle.Update;
  BackupItemSetScheduleHandle.Free;
end;
...
begin
  ...
  BackupItemAddFace.SetScheduleInfo( ScheduleType, ScheduleValue1, ScheduleValue2 );
  ...
  if not DesItemInfoReadUtil.ReadIsLocalDes( DesItemID ) then
    StorageApi.AddNetworkComputer( BackupPath, Format( ShowName_NetworkComputers, [MyNetPcInfoReadUtil.ReadName( NetworkDesItemUtil.getPcID( DesItemID ) )]), ItemSize );
end;
...
begin
  ...
  LocalBackupItemAddInfo.SetScheduleInfo( ScheduleType, ScheduleValue1, ScheduleValue2 );
...
procedure TBackupItemReadHandle.SetScheduleInfo(_ScheduleType, _ScheduleValue1,
...
begin
  ...
  LocalBackupItemAddXml.SetScheduleInfo( ScheduleType, ScheduleValue1, ScheduleValue2 );
...
begin
  ...
  MyBackupHandler.AddBackupJob( BackupPathInfo );
end;
...
begin
  ...
  FrmLocalDesAdd.SetShowName( DesItemID );
...
begin
  ...
  DesPath := NetworkDesItemUtil.getCloudPath( DesItemID );
  DesPcName := MyNetPcInfoReadUtil.ReadDesItemShow( DesItemID );
  if MyNetPcInfoReadUtil.ReadIsIos( PcID ) then
    DeviceType := DeviceType_IOS
  else
  if MyNetPcInfoReadUtil.ReadIsMac( PcID ) then
    DeviceType := DeviceType_Mac
  else
    DeviceType := DeviceType_Pc;

    // Moblie box 不显示
  if Pos( CloudName_MobileBox, DesPath ) > 0 then
    Exit;

  IsLan := MyNetPcInfoReadUtil.ReadIsLanPc( PcID );
  DesItemAddNetworkFace := TDesItemAddNetworkFace.Create( DesItemID );
  DesItemAddNetworkFace.SetDesInfo( PcName, DesPath );
  DesItemAddNetworkFace.SetIsOnline( IsOnline );
  DesItemAddNetworkFace.SetIsLan( IsLan );
  DesItemAddNetworkFace.SetAvailableSpace( AvailableSpace );
  DesItemAddNetworkFace.SetDeviceType( DeviceType );
  ...
  FrmBackupPcFilterAdd.AddChange;
    ...
    // 移动目标不显示
  if pos( FolderName_Ios, DesPath ) > 0 then
    Exit;

  FrmNetworkDesAdd := TFrmNetworkDesAdd.Create( DesItemID );
  FrmNetworkDesAdd.SetShowInfo( PcName, DesPath );
  FrmNetworkDesAdd.SetIsOnline( IsOnline );
  FrmNetworkDesAdd.SetIsLanPc( IsLan );
...
begin
  ...
  FrmNetworkDesIsOnline.SetIsLanPc( IsLanPc );
...
var
  FrmLocalDesRemove : TFrmNetworkDesRemove;
begin
  inherited;

  FrmLocalDesRemove := TFrmNetworkDesRemove.Create( DesItemID );
...
var
  FrmLocalSetAvailableSpace : TFrmNetworkSetAvailableSpace;
begin
  inherited;

    // 选择窗口空间信息
  FrmLocalSetAvailableSpace := TFrmNetworkSetAvailableSpace.Create( DesItemID );
...
class procedure BackupHintAppApi.SetBackupReal(IsBackupReal: Boolean);
var
  BackupSetReallyXml : TBackupSetReallyXml;
begin
  BackupSetReallyXml := TBackupSetReallyXml.Create;
  BackupSetReallyXml.SetIsRealBackup( IsBackupReal );
  BackupSetReallyXml.AddChange;
end;
...
begin
  if Params.BackupFileList.Count = 0 then  // 没有新文件备份
    Exit;
...
{ TBackupItemSetScheduleHandle }

procedure TBackupItemSetScheduleHandle.SetScheduleType(_ScheduleType: Integer);
begin
  ScheduleType := _ScheduleType;
end;

procedure TBackupItemSetScheduleHandle.SetScheduleValue(_ScheduleValue1,
  _ScheduleValue2: Integer);
begin
  ScheduleValue1 := _ScheduleValue1;
  ScheduleValue2 := _ScheduleValue2;
...
procedure TBackupItemSetScheduleHandle.SetToFace;
var
  BackupItemSetScheduleFace : TBackupItemSetScheduleFace;
begin
  BackupItemSetScheduleFace := TBackupItemSetScheduleFace.Create( DesItemID );
  BackupItemSetScheduleFace.SetBackupPath( BackupPath );
  BackupItemSetScheduleFace.SetSchduleType( ScheduleType );
  BackupItemSetScheduleFace.SetSchduleValue( ScheduleValue1, ScheduleValue2 );
  BackupItemSetScheduleFace.AddChange;
end;

procedure TBackupItemSetScheduleHandle.SetToInfo;
var
  BackupItemSetScheduleInfo : TBackupItemSetScheduleInfo;
begin
  BackupItemSetScheduleInfo := TBackupItemSetScheduleInfo.Create( DesItemID );
  BackupItemSetScheduleInfo.SetBackupPath( BackupPath );
  BackupItemSetScheduleInfo.SetSchduleType( ScheduleType );
  BackupItemSetScheduleInfo.SetSchduleValue( ScheduleValue1, ScheduleValue2 );
  BackupItemSetScheduleInfo.Update;
  BackupItemSetScheduleInfo.Free;
end;

procedure TBackupItemSetScheduleHandle.SetToXml;
var
  BackupItemSetScheduleXml : TBackupItemSetScheduleXml;
begin
  BackupItemSetScheduleXml := TBackupItemSetScheduleXml.Create( DesItemID );
  BackupItemSetScheduleXml.SetBackupPath( BackupPath );
  BackupItemSetScheduleXml.SetSchduleType( ScheduleType );
  BackupItemSetScheduleXml.SetSchduleValue( ScheduleValue1, ScheduleValue2 );
  BackupItemSetScheduleXml.AddChange;
end;

procedure TBackupItemSetScheduleHandle.Update;
begin
  SetToInfo;
  SetToFace;
  SetToXml;
