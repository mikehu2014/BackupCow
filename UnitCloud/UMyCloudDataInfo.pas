...
{$Region ' 数据读取 ' }
  ...
  public
    class function ReadMobileBoxPath : string;
  end;
...
class function MyCloudInfoReadUtil.ReadMobileBoxPath: string;
var
  CloudPathList : TStringList;
  i: Integer;
begin
  Result := '';
  CloudPathList := ReadCloudPathList;
  for i := 0 to CloudPathList.Count - 1 do
  begin
    if pos( 'Mobile', CloudPathList[i] ) <= 0 then
      Continue;
    Result := CloudPathList[i];
    Break;
  end;
  if ( Result = '' ) and ( CloudPathList.Count > 0 ) then
    Result := CloudPathList[0];
  CloudPathList.Free;
end;
