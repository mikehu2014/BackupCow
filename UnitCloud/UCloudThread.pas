...
{$Region ' 接收 云文件 ' }
  ...
  TNetworkReceiveCloudFileHandle = class( TNetworkReceiveFileHandle )
  private
    SendPcID : string;
  public
    procedure SetSendPcID( _SendPcID : string );
  protected    // 专为 Ios 文件监听设置
    function ReadCreateFilePath : string;override;
    procedure ReceiveFileAfter;override;
  end;
...
var
  CloudRecieveFileOperator : TCloudRecieveFileOperator;
  NetworkReceiveFileHandle : TNetworkReceiveCloudFileHandle;
begin
  CloudRecieveFileOperator := TCloudRecieveFileOperator.Create;
  NetworkReceiveFileHandle := TNetworkReceiveCloudFileHandle.Create;
  NetworkReceiveFileHandle.SetSendPcID( OwnerPcID );
...
{ TNetworkReceiveCloudFileHandle }

function TNetworkReceiveCloudFileHandle.ReadCreateFilePath: string;
begin
  if MyNetPcInfoReadUtil.ReadIsIos( SendPcID ) then
    Result := inherited + NotifyFilter_Name
  else
    Result := inherited;
end;

procedure TNetworkReceiveCloudFileHandle.ReceiveFileAfter;
var
  ChangeName, OrigiName : string;
begin
  if not MyNetPcInfoReadUtil.ReadIsIos( SendPcID ) then  // 非 ios 设备，不处理
    Exit;
  ChangeName := ReadCreateFilePath;   // 接收的文件名
  OrigiName := ReceiveFilePath;  // 原来的文件名
  RenameFile( ChangeName, OrigiName );  // 文件名替换
end;

procedure TNetworkReceiveCloudFileHandle.SetSendPcID(_SendPcID: string);
begin
  SendPcID := _SendPcID;
end;
