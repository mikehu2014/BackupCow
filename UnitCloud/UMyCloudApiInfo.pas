...
{$EndRegion}
  ...
  public
    class procedure AddMoblieBox;
    class procedure OpenMoblieBox( IosID : string );
  end;
...
var
  MoblieBox_IsExist : Boolean = False;
...
const
  ...
  CloudName_MobileBox = 'BackupCow.Mobile Box';
  FolderName_Ios = 'ios.Backup';
...
procedure TPcOnlineHandle.SendMoblieBoxToPc;
var
  MobilePath : string;
  AvailableSpace : Int64;
  CloudPathAddMsg : TCloudPathAddMsg;
begin
  MobilePath := MyCloudInfoReadUtil.ReadMobileBoxPath;

    // 可用空间
  AvailableSpace := MyHardDisk.getHardDiskFreeSize( MobilePath );

    // 发送路径
  CloudPathAddMsg := TCloudPathAddMsg.Create;
  CloudPathAddMsg.SetPcID( PcInfo.PcID );
  CloudPathAddMsg.SetCloudPath( MobilePath );
  CloudPathAddMsg.SetAvailableSpace( AvailableSpace );
  MyClient.SendMsgToPc( OnlinePcID, CloudPathAddMsg );

    // 发送恢复
  SendRestoreToPc( MobilePath );
end;
...
begin
  if MyNetPcInfoReadUtil.ReadIsIos( OnlinePcID ) then
    SendMoblieBoxToPc
  else
    SendCloudPathToPc;
end;
...
class procedure MyCloudPathUserApi.AddMoblieBox;
var
  CloudPath : string;
begin
  CloudPath := MyHardDisk.getBiggestHardDIsk + CloudName_MobileBox;
  AddItem( CloudPath );
end;

class procedure MyCloudPathUserApi.OpenMoblieBox( IosID : string );
var
  MobileBox : string;
begin
  MobileBox := MyCloudInfoReadUtil.ReadMobileBoxPath;
  MobileBox := MyFilePath.getPath( MobileBox ) + IosID;
  MobileBox := MyFilePath.getPath( MobileBox ) + 'ios_Driver';
  ForceDirectories( MobileBox );
...
begin
  AddToInfo;
    ...
    // 存在 mobilebox
  if Pos( CloudName_MobileBox, CloudPath ) > 0 then
