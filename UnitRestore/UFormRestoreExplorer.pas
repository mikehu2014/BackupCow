...
type
  ...
  TfrmRestoreExplorer = class(TForm)
    ...
    tbtnRecycle: TToolButton;
    tbtnSearchSearch: TToolButton;
    tbtnSearchRecycle: TToolButton;
    tbtnRecycleSearch: TToolButton;
    tbtnRecycleRecycle: TToolButton;
    ...
    procedure cbbSearchNameKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure vstSearchFileDblClick(Sender: TObject);
    ...
    procedure tbtnRecycleClick(Sender: TObject);
    procedure tbtnRecycleRecycleClick(Sender: TObject);
  private
    procedure SaveIni;
    procedure LoadIni;
  ...
  public
    ...
    procedure SetIsShowRoot( vst : TVirtualStringTree; IsShow : Boolean );
  private
...
begin
  ...
  tbtnRecyclePreview.Visible := False;
  tbtnRecycleSplit.Visible := False;
  tbtnSearchPreview.Visible := False;
  tbtnSearchSplit.Visible := False;
  tbtnSearch.Visible := not Params.IsFile;
  tbtnSearchSearch.Visible := not Params.IsFile;
  tbtnRecycleSearch.Visible := not Params.IsFile;
  tbtnRecycle.Visible := False;
  tbtnSearchRecycle.Visible := False;
  tbtnRecycleRecycle.Visible := False;
  col := vstSearchFile.Header.Columns[ VstExplorer_Recycled ];
  SetIsShowRoot( vstExplorer, not Params.IsFile );
  SetIsShowRoot( vstDeleteFile, not Params.IsFile );
  SetIsShowRoot( vstSearchFile, not Params.IsFile );
  if Params.HasDeleted then
    col.Options := col.Options + [ coVisible ]
  else
    col.Options := col.Options - [ coVisible ];
  tbExplorer.Visible := not Params.IsFile;
  tbRecycled.Visible := not Params.IsFile;
  tbSearch.Visible := not Params.IsFile;
...
procedure TfrmRestoreExplorer.SetIsShowRoot(vst: TVirtualStringTree;
  IsShow: Boolean);
begin
  if IsShow then
...
procedure TfrmRestoreExplorer.tbtnRecycleRecycleClick(Sender: TObject);
begin
  PcMain.ActivePage := tsExplorer;
end;
...
begin
  PcMain.ActivePage := tsSearchFile;
...
procedure TfrmRestoreExplorer.tbtnRecycleClick(Sender: TObject);
begin
  PcMain.ActivePage := tsDeletedFile;
  tbtnRecycleRecycle.Down := True;
end;
...
begin
  ...
  tbtnRecyclePreview.Visible := tbtnRecyclePreview.Enabled;
  tbtnRecycleSplit.Visible := tbtnRecyclePreview.Enabled;
...
procedure TfrmRestoreExplorer.vstSearchFileDblClick(Sender: TObject);
begin
  MyButton.Click( tbtnPreview );
end;
