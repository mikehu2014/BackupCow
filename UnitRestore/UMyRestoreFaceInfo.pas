...
{$Region ' 恢复删除文件 Explorer ' }
  ...
  TRestoreRecycleEmptyCheckFace = class( TRestoreDeleteExplorerChangeFace )
  protected
    procedure Update;override;
  end;
...
const
  ...
  RestoreIcon_PcOffline = 0;
  RestoreIcon_IphoneOnline = 7;
  RestoreIcon_MacOnline = 8;
  RestoreIcon_MyComputer = 9;
  RestoreIcon_NetworkFolder = 10;
  RestoreIcon_LanComputer = 11;
  RestoreIcon_InternetComputer = 12;
...
const
  ...
  RestoreStatusShow_Completed = 'Restore Completed';
...
begin
  RestoreDesData.MainName := PcName;
  RestoreDesData.DesName := Format( DesName_Show, [Path] );
  if IsLanPc then
    RestoreDesData.MainIcon := RestoreIcon_LanComputer
  else
    RestoreDesData.MainIcon := RestoreIcon_InternetComputer;
  RestoreDesData.NodeType := RestoreNodeType_NetworkDes;
end;

procedure TRestoreDesAddNetworkFace.SetIsLanPc(_IsLanPc: Boolean);
begin
  IsLanPc := _IsLanPc;
end;

procedure TRestoreDesAddNetworkFace.SetDesInfo( _PcName, _Path : string );
begin
  PcName := _PcName;
  Path := _Path;
end;
...
begin
  ...
  begin
    ...
    RestorePcBackupData.MainName := BackupPath;
...
begin
  ...
  begin
    ...
    frmMainForm.tbtnRestoreHistory.Down := True;
  end;
  if IsCompleted then
...
begin
  RestoreDesData.DesName := Format( DesName_Show, [DesItemID] );
  if MyNetworkFolderUtil.IsNetworkFolder( DesItemID ) then
  begin
    RestoreDesData.MainName := BackupShow_SharedFolder;
    RestoreDesData.MainIcon := RestoreIcon_NetworkFolder;
  end
  else
  begin
    RestoreDesData.MainName := BackupShow_MyComputer;
    RestoreDesData.MainIcon := RestoreIcon_MyComputer;
  end;
...
begin
  ...
  Result := MyHtmlHintShowStr.getHintRowNext( 'Backup Copy', NodeData.OwnerPcName + ' ' + Format( DesName_Show, [NodeData.RestorePath] ) );
  Result := Result + MyHtmlHintShowStr.getHintRow( 'Retore to',  'My Computer ' + Format( DesName_Show, [NodeData.SavePath]) );
  ...
  begin
    Result := Result + '<br />';
    Result := Result + MyHtmlHintShowStr.getHintRow( 'Deleted Files',  'Yes' );
  end;
end;
...
begin
  ...
  begin
    ...
    RestoreFrom := ParentData.MainName + ParentData.DesName;
  end;
  Result := MyHtmlHintShowStr.getHintRowNext( 'Backup Copy', NodeData.OwnerName + ' ' + Format( DesName_Show, [NodeData.MainName] ) );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( 'Storage',  ParentData.MainName + ParentData.DesName );
  ...
  Result := Result + MyHtmlHintShowStr.getHintRow( 'Keep Deleted/Replaced Files', MyBoolean.getBooleanStr( NodeData.IsSaveDeleted ) );
end;
...
{ TRestoreRecycleEmptyCheckFace }

procedure TRestoreRecycleEmptyCheckFace.Update;
var
  IsShowRecycle : Boolean;
...
begin
  ...
  IsShowRecycle := False;
  FirstNode := VstRestoreDeleteExplorer.RootNode.FirstChild;
  if Assigned( FirstNode ) then
  begin
    NodeData := VstRestoreDeleteExplorer.GetNodeData( FirstNode );
    IsShowRecycle := NodeData.IsFile or ( FirstNode.ChildCount > 0 );
  end;

  if IsShowRecycle then
  begin
    frmRestoreExplorer.tbtnRecycle.Visible := True;
    frmRestoreExplorer.tbtnSearchRecycle.Visible := True;
    frmRestoreExplorer.tbtnRecycleRecycle.Visible := True;
    if NodeData.IsFile then
    begin
      frmRestoreExplorer.tbExplorer.Visible := True;
      frmRestoreExplorer.tbRecycled.Visible := True;
      frmRestoreExplorer.tbSearch.Visible := True;
    end;
  end;
end;
