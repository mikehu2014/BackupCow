...
{$Region ' 数据结构 ' }
    ...
    // 恢复路径信息
  TRestoreJobInfo = class
  public
    RestorePath, OwnerID, RestoreFrom : string;
  public
    procedure SetItemInfo( _RestorePath, _OwnerID, _RestoreFrom : string );
  end;
  TRestoreJobList = class( TObjectList< TRestoreJobInfo > );

    // 恢复下载信息
  TRestoreDownloadInfo = class( TRestoreJobInfo )end;
  ...
  TRestoreDownloadList = class( TObjectList< TRestoreDownloadInfo > );
    ...
    // 高级路径信息
  TRestoreAdvancePathInfo = class( TRestoreJobInfo )
  public
    IsEncrypted : Boolean;
    PasswordExt : string;
  public
...
{$Region ' 恢复 父类 ' }
  ...
  protected
    procedure HandlePath( RestorePathInfo : TRestoreJobInfo );virtual;abstract;
  end;
  ...
  protected
    ...
    RestoreJobList : TRestoreJobList;
  protected
  ...
  public
    procedure AddRestoreJob( RestoreJobInfo : TRestoreJobInfo );
    function getRestoreJob : TRestoreJobInfo;
  end;
...
{$Region ' 恢复 下载 ' }
  ...
  private
    RestorePathInfo : TRestoreJobInfo;
    RestorePath, OwnerID, RestoreFrom : string;
  ...
  public
    constructor Create( _RestorePathInfo : TRestoreJobInfo );
    procedure Update;
  private
  ...
  protected
    procedure HandlePath( RestorePathInfo : TRestoreJobInfo );override;
  end;
...
procedure TRestoreExplorerProcessHandle.EmptyRecycleCheck;
var
  RestoreRecycleEmptyCheckFace : TRestoreRecycleEmptyCheckFace;
begin
  RestoreRecycleEmptyCheckFace := TRestoreRecycleEmptyCheckFace.Create;
  RestoreRecycleEmptyCheckFace.AddChange;
end;
...
constructor TRestoreStartHandle.Create(_RestorePathInfo: TRestoreJobInfo);
begin
...
procedure TRestoreJobInfo.SetItemInfo(_RestorePath, _OwnerID,
  _RestoreFrom: string);
begin
  RestorePath := _RestorePath;
  OwnerID := _OwnerID;
...
begin
    // 读取 下一个恢复
  RestorePathInfo := MyRestoreHandler.getRestoreJob;
...
procedure TMyRestoreHandler.AddRestoreJob(
...
begin
  ...
  ThreadLock.Enter;
    ...
    // 添加到列表中
  RestoreJobList.Add( RestoreJobInfo );
...
begin
  ...
  RestoreJobList := TRestoreJobList.Create;
  RestoreJobList.OwnsObjects := False;
...
begin
  ...
  RestoreJobList.OwnsObjects := True;
  RestoreJobList.Free;
  ThreadLock.Free;
  inherited;
end;
...
function TMyRestoreHandler.getRestoreJob: TRestoreJobInfo;
begin
  ThreadLock.Enter;
  if RestoreJobList.Count > 0 then
  begin
    Result := RestoreJobList[ 0 ];
    RestoreJobList.Delete( 0 );
  end
  else
  begin
    IsCreateThread := False;
