...
uses
  ...
  VCLTee.Series, VCLTee.TeEngine, VCLTee.TeeProcs, UMyUtil, VCLTee.Chart, htmlhint, uDebug,
...
type
  ...
  TfrmMainForm = class(TForm)
    ...
    tbtnMobileBox: TRzToolButton;
    tsMoblie: TRzTabSheet;
    plMobileBox: TPanel;
    tbMobileBox: TToolBar;
    tbtnShareMobileFile: TToolButton;
    tbtnRemoveMobile: TToolButton;
    vstMobileBox: TVirtualStringTree;
    tbtnShareMobileFolder: TToolButton;
    fdMobileBox: TOpenDialog;
    N3: TMenuItem;
    NetworkStatus2: TMenuItem;
    SignupaGroup1: TMenuItem;
    tbtnJobSetSplit: TToolButton;
    ToolButton11: TToolButton;
    tbtnRestoreHistory: TToolButton;
    pmNewRestore: TPopupMenu;
    ...
    Image6: TImage;
    Image7: TImage;
    tsStorage: TRzTabSheet;
    plStorage: TPanel;
    vstStorage: TVirtualStringTree;
    tbtnStorage: TRzToolButton;
    tbStorage: TToolBar;
    tbtnStorageExplorer: TToolButton;
    ...
    procedure btnMobileBoxClick(Sender: TObject);
    procedure tbtnShareMobileFileClick(Sender: TObject);
    procedure vstMobileBoxGetText(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
    procedure vstMobileBoxGetImageIndex(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
      var Ghosted: Boolean; var ImageIndex: Integer);
    procedure tbtnShareMobileFolderClick(Sender: TObject);
    procedure vstMobileBoxChange(Sender: TBaseVirtualTree; Node: PVirtualNode);
    procedure tbtnRemoveMobileClick(Sender: TObject);
    procedure NetworkStatus2Click(Sender: TObject);
    procedure SignupaGroup1Click(Sender: TObject);
    procedure tbtnRestoreHistoryClick(Sender: TObject);
    procedure FormDestroy(Sender: TObject);
    procedure NewRestoreMenuItemClick(Sender: TObject);
    procedure vstStorageGetText(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
    procedure vstStorageGetImageIndex(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
      var Ghosted: Boolean; var ImageIndex: Integer);
    procedure vstStoragePaintText(Sender: TBaseVirtualTree;
      const TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      TextType: TVSTTextType);
    procedure tbtnStorageExplorerClick(Sender: TObject);
    procedure vstStorageDblClick(Sender: TObject);
  public
  ...
  public
    ...
    class function ReadOnlineMobile : PVirtualNode;
  public
    class function getRestoreVisibleFirstChild : PVirtualNode;
    class procedure RefreshRestoreBtn;
  public
    class function RestoreFileHandle( FileNode : PVirtualNode ): Boolean;
    class function RestoreFolderHandle( FileNode : PVirtualNode ): Boolean;
  public
    class procedure IniBackupStorage;
  end;
...
const // 拖动文件
  ...
  DropFileType_MobileBox = 'MobileBox';

  DropFile_Hint = 'Drag and drop files here to start backup';
  DropFolder_Hint = 'Drag and drop folder here to start backup';

const
  ShowSotrage_MyComputer = 'Backups stored on My Computer';
  ShowSotrage_NetworkComputer = 'My Backups stored on Network Computer';
...
uses
  ...
  UFormAbout, UMobileBoxFace, UMobileBoxApi, UStorageApi,
  UAppEditionInfo, URegisterInfoIO, UFormBackupPcFilter, UFormRestorePcFilter,
  USettingInfo, UAppSplitEdition, UMyCloudApiInfo, UStorageFace,
...
begin
  ...
  vstMobileBox.Images := MyIcon.getSysIcon;
  vstStorage.Images := MyIcon.getSysIcon;
end;
...
begin
  ...
  vstMobileBox.PopupMenu := FormUtil.getPopMenu( tbMobileBox );
  vstStorage.PopupMenu := FormUtil.getPopMenu( tbStorage );
end;
...
begin
  ...
  vstMobileBox.NodeDataSize := SizeOf(TMobileBoxData);
  vstStorage.NodeDataSize := SizeOf(TVstStorageData);
end;
  ...
  for i := 0 to RestoreTempList.Count - 1 do
    ...
    begin
      if RestoreTempList[i].IsFile and not RestoreTempList[i].IsSaveDeleted then
        MainFormUtil.RestoreFileHandle( RestoreTempList[i].FileNode )
      else
        MainFormUtil.RestoreFolderHandle( RestoreTempList[i].FileNode );
      Break;
    end;
end;
...
procedure TfrmMainForm.btnMobileBoxClick(Sender: TObject);
var
  MobileBox, IosID : string;
  NodeData : PVstBackupData;
  MobileNode : PVirtualNode;
begin
  MobileNode := nil;

    // 选中的是否移动
  if Assigned( VstBackup.FocusedNode ) then
  begin
    NodeData := VstBackup.GetNodeData( VstBackup.FocusedNode );
    if NodeData.DeviceType = DeviceType_IOS then
      MobileNode := VstBackup.FocusedNode;
  end;

    // 是否存在在线的移动
  if not Assigned( MobileNode ) then
    MobileNode := MainFormUtil.ReadOnlineMobile;

    // 是否存在移动
  if Assigned( MobileNode ) then
  begin
    NodeData := VstBackup.GetNodeData( MobileNode );
    IosID := NetworkDesItemUtil.getPcID( NodeData.ItemID );
    MyCloudPathUserApi.OpenMoblieBox( IosID );
    Exit;
  end;

    // 打开根目录
  MobileBox := MyCloudInfoReadUtil.ReadMobileBoxPath;
  MyExplore.OpenFolder( MobileBox );
end;
...
procedure TfrmMainForm.SignupaGroup1Click(Sender: TObject);
begin
...
procedure TfrmMainForm.tbtnRemoveMobileClick(Sender: TObject);
var
  SelectNode : PVirtualNode;
  NodeData : PMobileBoxData;
begin
  SelectNode := vstMobileBox.GetFirstSelected;
  while Assigned( SelectNode ) do
  begin
    NodeData := vstMobileBox.GetNodeData( SelectNode );
    MobileBoxApi.RemoveItem( NodeData.Path );
    SelectNode := vstMobileBox.GetNextSelected( SelectNode );
  end;
end;
...
begin
  ...
  begin
    SelectNode := vstRestoreDown.RootNode.FirstChild;
    if not Assigned( SelectNode ) then  // 没有任何节点
      Exit;
  end;
  NodeData := vstRestoreDown.GetNodeData( SelectNode );
...
procedure TfrmMainForm.tbtnRestoreHistoryClick(Sender: TObject);
var
  IsHide : Boolean;
begin
  if tbtnRestoreHistory.Down then
  begin
    plRestoreDown.Visible := True;
    slRestoreDown.Visible := True;
  end
  else
  begin
    plRestoreDown.Visible := False;
    slRestoreDown.Visible := False;
  end;
end;
...
procedure TfrmMainForm.tbtnShareMobileFileClick(Sender: TObject);
var
  i : Integer;
begin
  if not fdMobileBox.Execute then
    Exit;

  for i := 0 to fdMobileBox.Files.Count - 1 do
    MobileBoxApi.AddItem( fdMobileBox.Files[i], FileExists( fdMobileBox.Files[i] ) );
end;

var
  Path_MobileBoxFolder : string = '';
procedure TfrmMainForm.tbtnShareMobileFolderClick(Sender: TObject);
begin
  if not MySelectFolderDialog.SelectNormal( ShowForm_SelectBackupFolder, '', Path_MobileBoxFolder ) then
    Exit;
  MobileBoxApi.AddItem( Path_MobileBoxFolder, FileExists( Path_MobileBoxFolder ) );
end;

procedure TfrmMainForm.tbtnStorageExplorerClick(Sender: TObject);
var
  SelectNode : PVirtualNode;
  NodeData : PVstStorageData;
begin
  SelectNode := vstStorage.GetFirstSelected;
  if not Assigned( SelectNode ) then
  begin
    if Assigned( vstStorage.RootNode.FirstChild ) and Assigned( vstStorage.RootNode.FirstChild.FirstChild ) then
      SelectNode := vstStorage.RootNode.FirstChild.FirstChild
    else
      Exit;
  end;
  NodeData := vstStorage.GetNodeData( SelectNode );
  MyExplore.OpenFolder( NodeData.ItemPath );
end;
...
begin
  ...
  begin
    if ( Node.Parent = Sender.RootNode ) and ( Column = VstBackup_BackupName ) then
      CellText := CellText + NodeData.DesName;
    Exit;
  end;
  ...
  begin
    if Column = VstBackup_BackupName then
        CellText := CellText + NodeData.MainName
    else
    if ( Column = VstBackup_FileSize ) and not Sender.Expanded[ Node ] then
    begin
  ...
  if ( Column = VstBackup_LastBackup ) and ( NodeData.LastSyncTime <> 0 ) then
    CellText := FormatDateTime( 'mm-dd  hh:nn', NodeData.LastSyncTime )
  else
  if Column = VstBackup_Percentage then
  begin
...
begin
  try   // 鼠标定位
    ...
    begin
      ...
      HintStr := '<font size="14">' + DropFile_Hint + '</font>';
  except
    HintStr := '';
  end;
...
begin
  ...
  begin
    NodeData := Sender.GetNodeData( Node );
    if ( ( NodeData.NodeType = BackupNodeType_LocalDes ) or NodeData.IsOnline ) and
       ( TextType = ttNormal )
    then
    begin
...
begin
  ...
  if IsShowRestore and IsSelected then
  begin
    tbtnRestoreSelected.Tag := 1;
  end
  else
    tbtnRestoreSelected.Tag := 0;
...
begin
  if Assigned( vstRestoreShow.FocusedNode ) and
     ( vstRestoreShow.FocusedNode.Parent <> vstRestoreShow.RootNode )
  then
    MyButton.Click( tbtnRestoreSelected );
end;

procedure TfrmMainForm.vstMobileBoxChange(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
begin
  tbtnRemoveMobile.Enabled := Sender.SelectedCount > 0;
end;

procedure TfrmMainForm.vstMobileBoxGetImageIndex(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
  var Ghosted: Boolean; var ImageIndex: Integer);
var
  NodeData : PMobileBoxData;
...
procedure TfrmMainForm.vstMobileBoxGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: string);
var
  NodeData : PMobileBoxData;
begin
  NodeData := Sender.GetNodeData( Node );
  if Column = VstMobileBox_Path then
...
begin
  if tbtnRestoreDownRun.Enabled then
    tbtnRestoreDownRun.Click
  else
...
begin
  ...
  if TextType = ttStatic then
  begin
    if ( Node.Parent = Sender.RootNode ) and ( Column = VstRestore_RestoreName ) then
  ...
  try
    ...
    if Column = VstRestore_LastBackupTime then
      CellText := formatdatetime('mm-dd  hh:mm', NodeData.LastBackupTime );
  except
  end;
end;
...
procedure TfrmMainForm.vstRestoreShowPaintText(Sender: TBaseVirtualTree;
  ...
  TextType: TVSTTextType);
begin
  if ( Node.Parent = Sender.RootNode ) and ( Column = VstBackup_BackupName ) and ( TextType = ttNormal ) then
  begin
    TargetCanvas.Font.Style := TargetCanvas.Font.Style + [fsBold];
    TargetCanvas.Font.Size := 10;
  end;
...
procedure TfrmMainForm.vstStorageDblClick(Sender: TObject);
begin
  MyButton.Click( tbtnStorageExplorer );
end;

procedure TfrmMainForm.vstStorageGetImageIndex(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
  var Ghosted: Boolean; var ImageIndex: Integer);
var
  NodeData : PVstStorageData;
begin
  ...
  if Node.Parent = Sender.RootNode then
  begin
    if Column = VstStorage_ShowName then
    begin
      if Kind = ikState then
...
procedure TfrmMainForm.vstStorageGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: string);
var
  NodeData : PVstStorageData;
begin
  ...
  if Column = VstStorage_Size then
  begin
    if Node.Parent = Sender.RootNode then
      CellText := ''
    else
...
procedure TfrmMainForm.vstStoragePaintText(Sender: TBaseVirtualTree;
  const TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
  TextType: TVSTTextType);
var
  NodeData : PVstStorageData;
begin
  if ( Node.Parent = Sender.RootNode ) and ( Column = VstStorage_ShowName ) then
  begin
    TargetCanvas.Font.Style := TargetCanvas.Font.Style + [fsBold];
    TargetCanvas.Font.Size := 10;
  end;
...
procedure TDropFileHandle.AddFileMobile;
var
  i: Integer;
begin
  for i := 0 to FilePathList.Count - 1 do
    MobileBoxApi.AddItem( FilePathList[i], FileExists( FilePathList[i] ) );
end;
...
begin
  ...
  if frmMainForm.PcMain.ActivePageIndex = MainPage_Mobile then
    DropFileType := DropFileType_MobileBox;
end;
...
begin
  ...
  if DropFileType = DropFileType_MobileBox then
    AddFileMobile;
...
procedure TAddDropBackupFile.AddToMobileBox;
var
  i: Integer;
begin
  for i := 0 to FilePathList.Count - 1 do
    if FileExists( FilePathList[i] ) then
    begin
      MobileBoxApi.AddItem( FilePathList[i], FileExists( FilePathList[i] ) );
      frmMainForm.PcMain.ActivePage := frmMainForm.tsMoblie;
      frmMainForm.tbtnMobileBox.Visible := True;
      frmMainForm.tbtnMobileBox.Down := True;
    end;
end;
...
begin
  ...
  begin
    ...
    if not Assigned( SelectNode ) then  // 没有选中节点，选择默认节点
    begin
      DesItemID := ReadDefaultDes;
      if DesItemID <> '' then
        DesItemList.Add( DesItemID );
      Exit;
    end;
    NodeData := VstBackup.GetNodeData( SelectNode );
    if NodeData.DeviceType = DeviceType_IOS then  // 选中 IOS
    begin
      IsMobileBox := True;
      IosID := NetworkDesItemUtil.getPcID( NodeData.ItemID );
      Exit;
    end;
    if SelectNode.Parent = VstBackup.RootNode then  // 选中父节点
      DesItemList.Add( NodeData.ItemID )
    else
    if Assigned( SelectNode.Parent ) and ( NodeData.NodeType <> BackupNodeType_ErrorItem ) then   // 选中子节点
    begin
      ParentData := VstBackup.GetNodeData( SelectNode.Parent );
      DesItemList.Add( ParentData.ItemID );
    end;
  end;
end;

function TAddDropBackupFile.ReadDefaultDes: string;
var
  VstBackup : TVirtualStringTree;
  SelectNode : PVirtualNode;
  NodeData : PVstBackupData;
begin
  ...
  while Assigned( SelectNode ) do
  begin
    if VstBackup.IsVisible[ SelectNode ] then
    begin
      if Result <> '' then  // 存在多个可见路径
      begin
        Result := '';
        Break;
      end;
...
//  BackupItemUserApi.SetAutoSyncInfo( Params );
  ...
  if ( NewBackupConfigInfo.ScheduleType = OldBackupConfigInfo.ScheduleType ) and
     ( NewBackupConfigInfo.ScheduleValue1 = OldBackupConfigInfo.ScheduleValue1 ) and
     ( NewBackupConfigInfo.ScheduleValue2 = OldBackupConfigInfo.ScheduleValue2 )
  then
    Exit;

  Params.DesItemID := DesItemID;
  Params.BackupPath := BackupPath;
  Params.ScheduleType := NewBackupConfigInfo.ScheduleType;
  Params.ScheduleValue1 := NewBackupConfigInfo.ScheduleValue1;
  Params.ScheduleValue2 := NewBackupConfigInfo.ScheduleValue2;
  BackupItemUserApi.SetScheduleInfo( Params );
...
begin
  Result := MainFormUtil.RestoreFileHandle( Node );
end;

function TRestoreAllFileHandle.FolderHandle(Node: PVirtualNode): Boolean;
begin
  Result := MainFormUtil.RestoreFolderHandle( Node );
end;

procedure TRestoreAllFileHandle.RestoreDefaultNode;
begin
  MainFormUtil.RefreshRestoreBtn;
  with frmMainForm do
  begin
    tbtnRestoreSelected.Down := True;
    tbtnRestoreSelected.CheckMenuDropdown;
    tbtnRestoreSelected.Down := False;
  end;
end;
...
begin
  ...
  if frmMainForm.tbtnRestoreSelected.Tag = 0 then  // 没有选择恢复节点
  begin
    RestoreDefaultNode;
    Exit;
  end;
...
begin
  ...
  RestoreFromName := ParentData.MainName + ParentData.DesName;
...
begin
  ...
  Params.RestorePath := NodeData.ItemID;
  Params.OwnerName := NodeData.OwnerName;
  Params.RestoreFromName := ParentData.MainName + ParentData.DesName;
...
begin
  ...
  if MainPage = MainPage_Mobile then
  begin
    frmMainForm.tbtnMobileBox.Visible := True;
    frmMainForm.tbtnMobileBox.Down := True
  end
  else
  if MainPage = MainPage_Storage then
  begin
    frmMainForm.tbtnStorage.Visible := True;
    frmMainForm.tbtnStorage.Down := True;
  end;
...
class function MainFormUtil.getRestoreVisibleFirstChild: PVirtualNode;
var
  vstRestoreShow : TVirtualStringTree;
  SelectNode : PVirtualNode;
begin
  Result := nil;
  vstRestoreShow := frmMainForm.vstRestoreShow;
  SelectNode := vstRestoreShow.GetFirstVisible;
  if Assigned( SelectNode ) then
    Result := vstRestoreShow.GetFirstVisible( SelectNode );
end;

class procedure MainFormUtil.IniBackupStorage;
begin
  StorageApi.Add( RootItemID_MyComputer, ShowSotrage_MyComputer );
  StorageApi.Add( RootItemID_NetworkComputer, ShowSotrage_NetworkComputer );
end;

class function MainFormUtil.ReadOnlineMobile: PVirtualNode;
var
  vstBackup : TVirtualStringTree;
  SelectNode : PVirtualNode;
  NodeData : PVstBackupData;
begin
  ...
  begin
    ...
    SelectNode := vstBackup.GetNextVisible( SelectNode );
  end;
end;

class procedure MainFormUtil.RefreshRestoreBtn;
var
  SelectNode, ChildNode : PVirtualNode;
  NodeData, ChildData : PVstRestoreData;
...
begin
  ...
  begin
    ...
    pmNewRestore.Items.Clear;
    SelectNode := vstRestoreShow.RootNode.FirstChild;
    while Assigned( SelectNode ) do
    begin
      NodeData := vstRestoreShow.GetNodeData( SelectNode );
      if vstRestoreShow.IsVisible[ SelectNode ] then
      begin
        Mi := TMenuItem.Create( pmNewRestore );
        Mi.Caption := NodeData.MainName + NodeData.DesName;
        ChildNode := SelectNode.FirstChild;
        while Assigned( ChildNode ) do
        begin
          if vstRestoreShow.IsVisible[ ChildNode ] then
          begin
            ChildData := vstRestoreShow.GetNodeData( ChildNode );
            ChildMi := TMenuItem.Create( pmNewRestore );
            ChildMi.Caption := ChildData.MainName;
            ...
            RestoreTempInfo.SetRestoreInfo( ChildData.IsFile, ChildData.IsSaveDeleted );
            RestoreTempList.Add( RestoreTempInfo );
          end;
          ChildNode := ChildNode.NextSibling;
        end;
        pmNewRestore.Items.Add( Mi );
        Inc( OneLevel );
      end;
      SelectNode := SelectNode.NextSibling;
    end;
...
class function MainFormUtil.RestoreFileHandle(FileNode: PVirtualNode): Boolean;
var
  RestoreNowSelectHandle : TRestoreNowSelectHandle;
begin
  RestoreNowSelectHandle := TRestoreNowSelectHandle.Create( FileNode );
  Result := RestoreNowSelectHandle.Update;
  RestoreNowSelectHandle.Free;
end;

class function MainFormUtil.RestoreFolderHandle(FileNode: PVirtualNode): Boolean;
var
  RestoreExplorerSelectHandle : TRestoreExplorerSelectHandle;
begin
  RestoreExplorerSelectHandle := TRestoreExplorerSelectHandle.Create( FileNode );
...
begin
  ...
  begin
    if pmBackupPc.Items[i].Tag = -1 then
      Continue;
