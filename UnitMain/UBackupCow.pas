...
uses
     ...
     UMobileBoxInfo, UMobileBoxXml, UMainApi, UStorageApi,
...
procedure TBackupCowCreate.CreateMobileBox;
begin
  MyMobileBox := TMyMobileBox.Create;
end;
...
procedure TBackupCowCreate.ShowMobileBox;
begin
  if MoblieBox_IsExist then
...
procedure TBackupCowCreate.CreateStorage;
begin
  MainFormUtil.IniBackupStorage
end;
...
//  MyTimerHandler.AddTimer( HandleType_MarkAppRunTime, 1 );  // 记录程序运行信息
end;
...
procedure TBackupCowDestory.DestoryMobileBox;
begin
  MyMobileBox.Free;
end;
...
var
  CloudPath : string;
begin
  CloudPath := MyHardDisk.getBiggestHardDIsk + 'BackupCow.Storage';
...
var
  LocalDesPath : string;
begin
  LocalDesPath := MyHardDisk.getBiggestHardDIsk + 'BackupCow.Storage';
...
procedure TMyXmlReadHandle.ReadMobileBoxXml;
var
  MoblieBoxXmlReader : TMoblieBoxXmlReader;
begin
  try
    MoblieBoxXmlReader := TMoblieBoxXmlReader.Create;
    MoblieBoxXmlReader.Update;
    MoblieBoxXmlReader.Free;
  except
  end;
end;
...
begin
  ...
  AdsShowCount := IntToStr( MyRegisterItem_AdsShowCount );
    ...
    // 状态信息
  if Read_IsPortMap then  // 是否已经端口映射
    StatusStr := 'PortMapOK'
  else
    StatusStr := 'PortMapError';
  StatusStr := StatusStr + HttpMarkApp_StatusSplit;
  if Backup_Really then
    StatusStr := StatusStr + 'Backuped'
  else
    StatusStr := StatusStr + 'NoBackup';
  BackupHintAppApi.SetBackupReal( False );
  StatusStr := StatusStr + HttpMarkApp_StatusSplit + MyEditionUtil.get;
  ...
  params.Add( HttpMarkApp_Account + '=' + AppSplitEditionUtl.ReadAccount );
  params.Add( HttpMarkApp_Status + '=' + StatusStr );
  ...
  params.free;
    ...
    // 一天后再运行
  MyTimerHandler.AddTimer( HandleType_MarkAppRunTime, 60 * 60 * 24 );  // 记录程序运行信息
end;
